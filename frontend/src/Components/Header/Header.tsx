...
//             <div className=" xs-mx:hidden text-3xl font-semibold">Expora</div>
//         </div>
//         {NavLinks()}
    ...
    <div data-aos="zoom-out" className="w-full bg-mine-shaft-950 px-6 text-white h-20 flex justify-between items-center font-['poppins']">
      ...
      <div className="flex gap-3 items-center">
        {user ? (
          ...
          <Link to="/login" className="text-mine-shaft-200 hover:text-bright-sun-400">
          ...
          <div className="flex flex-col gap-6 items-center">
            {links.map((link, index) => (
              <div key={index} className="h-full flex items-center">
                <div className="hover:text-bright-sun-400 text-xl" onClick={() => handleClick(link.url)}>
                  {link.name}
                </div>
              </div>
            ))}
